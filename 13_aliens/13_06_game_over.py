# 1: straightforward
# To make it quicker to test, there will only be one life, which can be lost by
# colliding with an enemy, or if an enemy reaches the left side.
import sys
import pygame
    ...
    def run_game(self):
        ...
        while True:
            ...
            if not self.game_active:
                # Game over messages
                if self.counter == 0:
                    print("You eliminated...oh, none. Better luck next time...")
                else:
                    print(f"You eliminated {self.counter} enemies!")
                quit()
